#' This functions cleans and joins the data on estimated demand shortages. 
#'
#' @param new_cea_files a list of the filenames of the csv tables converted from 
#' the pdf reports downloaded from the CEA website.
#' @param min_year numeric: first year of the sample I'm interested in.
#' @param old_avg_raw data frame: energy data from on avg shortages from Alcott et al.
#' @param old_peak_raw data frame: energy data from on peak shortages from Alcott et al.
#' @param avg_2003_data data frame: the year 2003 is (for some reason) not inlcuded in 
#' the alcott data, but is present in the scans. This file is the data manually entered
#' by me. 
#' @param peak_2003_data same as above but with the peak-data (is all NA values).
#' 
#' @return data frame with the cleaned energy shortages data.

# TODO fill in missing rows from allcott:
