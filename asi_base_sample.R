# This scripts takes the cleaned blocks of the ASI data, removes observations
...
block_list <- tibble(filename = cleaned_files) %>%
	mutate(
		data = map(filename, readRDS),
		file = str_match(
			string = filename,
			pattern = "block_[A-Z]"
			) %>% as.character()
...
conc_path <- here("data/external/concord_tables/asi_state_codes/state_codes_table.csv")
state_conc_tbl <- read_csv(conc_path) %>%
	select(
...
name_path <- here("data/external/concord_tables/asi_state_codes/state_names_1617.csv")
state_name_tbl <- read_csv(name_path)
...
# Write unweighted files -----------------------------------------

print("Writing unweighted files.")
