...
\begin{figure}[htpb]
	...
	\caption{Production disruptions tend to have a higher frequence in poorer, less complex economies. The color of points are country economic comeplexity index values (ECI), where blue is more complex. Gray crosses has a missing complexity value. Lines are SLR fits to ln(GDP/cap). Disruption data is from \cite{world_bank_enterprise_2020}. Countries are not surveyed the same years, so complexity and GDP/cap values have been matched to the newest survey year for each country before 2017. No country has more than one observation.}
...
\item What relationship, if any, does this association between complexity and interruptions have with the long-run complexity of the manufacturing sector in India?
...
Taken together, the two points highlighed thus far suggests an important path-way between the complexity of plants and interruptions in the production environment.  We would expect more primary production (that is, less complex production) in more disruptive environments if A) more complex products 

re complex products have longer production chains, they are punished harder by supply-chain unreliability, and B) that longer production chains will tend to have their later stages in more reliable environments, we would then expect to see more primary production (that is, less complex production) in more disruptive environments. This is corresponds well with the picture in figure \ref{fig:disruption-fig}. 
...
To sum up, I highlight three important interactions between interuptions and economic complexity at the production-level:

\begin{enumerate}

\item Production failures are more costly the longer down the production chain it happens. While some of this effect should be observed in plant revenues due to the higher price of inputs, this value loss will act more in the pre-production choice of where to place production, especially so for firms controlling a larger share of the whole value chain.
...
In this section I first develop the predictions from the discussion in section ... into a set of testable hypotheses. I outline the empirical strategy in section \ref{emp_strat}. FinallyI present the construction of my key variables.

\subsection{Hypotheses}
Based on the possible relationship between product complexity and production interruptions discussed in the previous section, I identify three hypotheses on the relationship between economic complexity at the plant level and the electricity shortages measure at the state level. 

If ... I would expect.. Specified as a null hypothesis.
...
I now turn my main analysis: how are states' electricity shortages related to the sophistication of production of plants? 
...
Indexing plants by $f$, years by $t$, and states by $s$ this gives me my two main equations. 
...
Table \ref{tab:interaction_avg_pci} presents the result from the regressions (column (4) and (6)) as well as a number of alternatives. Table \ref{tab:interaction_max_pci} presents the same results, but using $C^{max}$ in place of $C$.
...
% ------------ WRITING ------------ 
I now turn to the effect of longer-run shortages on the distribution of factories. Are plants with a more complex production output less likely to be constructed in states that have unreliable electricity? 
...
% ----------- NOTES ---------------
 - TODO results: One of the issues is that the industry a plant belongs to explains a ton of the variance in complexity (see column 1). This entails that whether or not complexity ...
...
\section{Calculating product complexity}%
